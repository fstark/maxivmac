...
|-------|-------------|--------|
| 1 | Header, types, empty class skeleton | |
| 2 | Catalog scan + query methods + tests | |
| 3 | File/directory creation + deletion + tests | |
| 4 | Move/rename + metadata (SetFileInfo) + tests | |
| 5 | Data fork I/O + tests | |
| 6 | Resource fork I/O + tests | |
| 7 | TEXT fork I/O + conversion stats + tests | |
| 8 | Working directories + volumeStats + tests | |
| 9 | Build integration (CMakeLists.txt, main exe) | |
| 10 | Rewire extn_extfs.cpp read-only commands | |
| 11 | Rewire extn_extfs.cpp write commands | |
| 12 | Remove dead code from extn_extfs.cpp | |
...
<parameter name="content"># HostVolume — Implementation Plan
...
- `namespace storage`
- `enum class FMErr` with all error codes (`kNoErr`, `kFnfErr`,
  `kDupFNErr`, `kParamErr`, `kRfNumErr`, `kIoErr`, `kDirNFErr`,
  `kFBsyErr`, `kWPrErr`)
- `struct CatalogEntry` with all fields (cnid, parentDirID, isDirectory,
  hostPath, macName, type, creator, finderFlags, dataForkSize,
  rsrcForkSize, crDate, modDate, isText)
...
- `class HostVolume` with all public method declarations:
  ...
  - `openFork()`, `readFork()`, `writeFork()`, `closeFork()`
  - `openWD()`, `wdToDirID()`, `closeWD()`
  - `struct TextStats { conversions, bytesIn, bytesOut }`
  - `textConversionStats()`, `resetTextConversionStats()`
- Private members:
  - `rootPath_`, `mounted_`, `catalog_`, `nextCNID_`
  - `struct OpenFork { cnid, fork, fp }`, `openForks_`, `nextHandle_`
  - `wdTable_`, `nextWD_`
...
- [ ] `src/storage/host_volume.cpp` exists with all stubs
- [ ] `test/test_host_volume.cpp` exists with smoke test
- [ ] Build clean (add files to CMake test target)
- [ ] Commit: `"hostvolume: phase 1 — header, types, skeleton"`
...
**`findByName(parentDirID, macName)`** — linear scan, compare
`parentDirID` and case-insensitive compare of `macName` (compare each
byte via `tolower()`).
...
Add to `test/test_host_volume.cpp`.  Each test creates a temporary
directory tree under `fs::temp_directory_path() / "test_hv"`, mounts
a `HostVolume`, and removes the tree afterward.
...
1. **"mount empty directory"** — `isMounted()`, `childCount(kRootDirID) == 0`.

2. **"mount with files"** — create `hello.txt` + `image.png` →
   `childCount == 2`, `findByName` locates both.

3. **"mount with subdirectory"** — dir with subdir containing one file
   → subdir `isDirectory`, its `childCount == 1`.

4. **"sidecar files hidden"** — create `test.txt` and `._test.txt` →
   only `test.txt` in catalog.

5. **"hidden files skipped"** — create `.hidden` → not in catalog.

6. **"filename decoding"** — host file `^3Acolon` → macName `:colon`.

7. **"findByName case-insensitive"** — find `"hello.txt"` when file
   is `Hello.txt`.

8. **"findByCNID not found"** — returns nullptr.

9. **"nthChild out of range"** — returns nullptr.

10. **"metadata from extension"** — `readme.txt` → type `TEXT`,
    creator `ttxt`.

11. **"metadata from sidecar"** — create file with AppleDouble sidecar
    containing custom type/creator → catalog reflects sidecar values.

12. **"mount nonexistent"** — returns false, not mounted.

### Fence

- [ ] `mount()` scans host directory tree via AppleDouble
- [ ] All four query methods work
- [ ] `resolveParentPath()` works for root and subdirectories
- [ ] 12 test cases pass
- [ ] Full build clean, all existing tests still pass
- [ ] Commit: `"hostvolume: phase 2 — catalog scan and queries"`
...
2. `resolveParentPath()` → `kDirNFErr` if empty.
3. `appledouble::HostNameFromMac(macName)` → host filename.
4. `fopen(hostPath, "wb")` → create empty file, `fclose()`.
5. Build `CatalogEntry`, push to `catalog_`.
6. Return new CNID.

Use a file-scope `currentMacDate()` helper (or private static method):
...
Same as `createFile()` but `fs::create_directory()` and
`isDirectory = true`.
...
1. `findByName()` → `kFnfErr`.
2. Directory + children → `kFBsyErr`.  Directory empty → `fs::remove()`.
3. File → `appledouble::DeleteWithSidecar()`.
4. Erase from catalog by CNID.

### 3.4 — Tests

1. **"createFile basic"** — mount, create → CNID > 0, host file exists,
   `findByName` finds it.

2. **"createFile duplicate"** → `kDupFNErr`, CNID == 0.

3. **"createFile escaped name"** — Mac name `:special` → host file
   `^3Aspecial`.

4. **"createFile bad parent"** → `kDirNFErr`.

5. **"createDir basic"** — host dir exists, `isDirectory == true`.

6. **"remove file"** — host file gone, not in catalog.

7. **"remove file with sidecar"** — set custom type (creates sidecar),
   remove → both gone.

8. **"remove non-empty directory"** → `kFBsyErr`.

9. **"remove empty directory"** → succeeds.

10. **"remove non-existent"** → `kFnfErr`.

### Fence

- [ ] `createFile()` creates host file + catalog entry
- [ ] `createDir()` creates host directory + catalog entry
- [ ] `remove()` handles files (with sidecar), dirs, and errors
- [ ] 10 test cases pass
- [ ] Full build clean, all existing tests still pass
- [ ] Commit: `"hostvolume: phase 3 — create and delete"`
...
1. `findByName(srcDirID, macName)` → `kFnfErr`.
2. `resolveParentPath(dstDirID)` → `kFnfErr` if empty.
3. Compute `newHostPath = dstPath / filename`.
4. Directory → `fs::rename()`.  File → `appledouble::RenameWithSidecar()`.
5. Update moved entry's `parentDirID` and `hostPath`.
6. Directory → update all descendant `hostPath` by rewriting prefix.

Private helper `updateDescendantPaths(oldPrefix, newPrefix)` — linear
scan, for each entry whose `hostPath` starts with `oldPrefix + "/"`,
replace the prefix.

### 4.2 — Implement `setFileInfo()`

1. `mutableFindByCNID(cnid)` → `kFnfErr`.
2. `appledouble::SetFinderInfo(entry.hostPath, {type, creator, entry.finderFlags})`.
3. Update `entry.type` and `entry.creator`.
4. If type changed to/from `TEXT`, update `entry.isText` and call
   `invalidateTextSize(entry)`.

### 4.3 — Implement `invalidateTextSize()`

If `entry.isText`: `entry.dataForkSize = appledouble::MacRomanSizeFromUTF8File(entry.hostPath)`.
If not: `entry.dataForkSize = fs::file_size(entry.hostPath)`.

### 4.4 — Tests

1. **"move file between dirs"** — file moves, parentDirID updated.

2. **"move file with sidecar"** — sidecar also moved.

3. **"move directory with children"** — child hostPath prefix updated.

4. **"move non-existent"** → `kFnfErr`.

5. **"setFileInfo basic"** — catalog updated, sidecar created on disk.

6. **"setFileInfo to default removes sidecar"** — set custom, set back
   to default → sidecar removed.

7. **"setFileInfo updates isText"** — set type to TEXT → `isText` true.

### Fence

- [ ] `move()` relocates file/dir + sidecar, updates descendants
- [ ] `setFileInfo()` persists metadata via AppleDouble
- [ ] `invalidateTextSize()` recaches TEXT size
- [ ] 7 test cases pass
- [ ] Full build clean, all existing tests still pass
- [ ] Commit: `"hostvolume: phase 4 — move and metadata"`
...
Implement fork open/read/write/close for non-TEXT data forks.

### 5.1 — Implement `openFork()` (data fork path)

1. `findByCNID(cnid)` → `kFnfErr` if not found or is directory.
2. If `fork == ForkType::Data`:
   - `fopen("r+b")`, fallback to `fopen("rb")`, fallback `fopen("w+b")`.
   - Seek to end → size → seek to start.
   - Store `OpenFork{cnid, Data, fp}` in map.
3. `outSize` = file size.
4. Return handle.

### 5.2 — Implement `readFork()` (data, non-TEXT)

1. Lookup handle → `kRfNumErr`.
2. `fseek`, `fread` into `buf`.
3. `outRead` = bytes read.

### 5.3 — Implement `writeFork()` (data, non-TEXT)

1. Lookup handle.
2. `fseek`, `fwrite`, `fflush`.
3. Update `entry.dataForkSize` (seek end, ftell).
4. Update `entry.modDate`.
5. `outWritten` = bytes written.

### 5.4 — Implement `closeFork()`

1. Lookup handle.
2. If `fp` non-null: `fclose(fp)`.
3. Erase from map.

### 5.5 — Tests

1. **"open/close data fork"** — handle > 0, size == 0.  Close OK.

2. **"write then read"** — write "hello", reopen, read → "hello".

3. **"read at offset"** — write "hello world", read 5 at offset 6
   → "world".

4. **"write updates catalog size"** — 100 bytes → `dataForkSize == 100`.

5. **"open non-existent CNID"** → `kFnfErr`.

6. **"read bad handle"** → `kRfNumErr`.

7. **"write updates modDate"** — modDate is recent.

### Fence

- [ ] Data fork open/read/write/close works for non-TEXT files
- [ ] Catalog updated after writes
- [ ] 7 test cases pass
- [ ] Full build clean, all existing tests still pass
- [ ] Commit: `"hostvolume: phase 5 — data fork I/O"`
...
### 6.1 — `openFork()` resource path

1. `findByCNID()` → `kFnfErr`.
2. `handle = nextHandle_++`.
3. `openForks_[handle] = { cnid, Resource, nullptr }`.
4. `outSize = appledouble::ResourceForkSize(entry.hostPath)`.

### 6.2 — `readFork()` resource path

1. `appledouble::ReadResourceFork(entry.hostPath, offset, buf.size())`.
2. Copy to `buf`, set `outRead`.

### 6.3 — `writeFork()` resource path

1. `appledouble::WriteResourceFork(entry.hostPath, offset, data)`.
2. Update `entry.rsrcForkSize`.
3. Update `entry.modDate`.
4. `outWritten = data.size()`.

### 6.4 — Tests

1. **"open resource fork (no sidecar)"** — size == 0.

2. **"write then read resource fork"** — round-trip matches.

3. **"resource fork creates sidecar"** — sidecar appears on disk.

4. **"resource fork at offset"** — overlapping writes resolved correctly.

5. **"resource fork updates rsrcForkSize"** — catalog matches.

### Fence

- [ ] Resource fork I/O via AppleDouble library
- [ ] Sidecar created automatically on write
- [ ] Catalog updated after writes
- [ ] 5 test cases pass
- [ ] Full build clean, all existing tests still pass
- [ ] Commit: `"hostvolume: phase 6 — resource fork I/O"`
...
In `readFork()`, when `entry.isText && of.fork == Data`:
...
In `writeFork()`, when `entry.isText && of.fork == Data`:
...
### 7.3 — TEXT `openFork()` size

For TEXT files, `outSize` should be `entry.dataForkSize` (the cached
Mac Roman byte count), not the host file size.  Still open a `FILE*`
for consistency.

### 7.4 — Stats accessors

Already stubbed — now return/reset `textStats_`.

### 7.5 — Tests

1. **"TEXT read converts UTF-8"** — file with UTF-8 `"café"` (5 bytes),
   type TEXT → `dataForkSize == 4`, read returns Mac Roman bytes.

2. **"TEXT read at offset"** — read partial → correct slice.

3. **"TEXT write converts to UTF-8"** — write Mac Roman, read host file
   → valid UTF-8.

4. **"TEXT conversion stats"** — 3 reads → `conversions == 3`,
   `bytesOut == 3 × macRomanSize`.

5. **"reset text stats"** → all zero.

6. **"TEXT dataForkSize in catalog"** — Mac Roman count, not UTF-8.

### Fence

- [ ] TEXT reads convert UTF-8 → Mac Roman
- [ ] TEXT writes convert Mac Roman → UTF-8
- [ ] `TextStats` tracks conversions, bytesIn, bytesOut
- [ ] 6 test cases pass
- [ ] Full build clean, all existing tests still pass
- [ ] Commit: `"hostvolume: phase 7 — TEXT conversion + stats"`
...
### 8.1 — Implement WD methods

```cpp
uint32_t HostVolume::openWD(uint32_t dirID) {
    uint32_t ref = nextWD_++;
    wdTable_[ref] = dirID;
    return ref;
}
...
1. **"openWD / wdToDirID round-trip"** → correct dirID.

2. **"closeWD"** → `wdToDirID` returns 0.

3. **"multiple WDs"** → distinct refs, both resolve.

4. **"volumeStats"** — 3 files totaling N bytes → correct counts.

5. **"volumeStats empty"** → both 0.

### Fence

- [ ] WD open/resolve/close functional
- [ ] `volumeStats()` correct
- [ ] 5 test cases pass
- [ ] Full build clean, all existing tests still pass
- [ ] Commit: `"hostvolume: phase 8 — working dirs and volume stats"`
...
Wire `host_volume.cpp` into both the main executable and the test
target.  Until now, the new files may have been added temporarily.

### 9.1 — Add storage sources to `MINIVMAC_SOURCES`

In `CMakeLists.txt`, add **after the existing core sources** (the
storage sources are currently only in the test target — they must also
be in the main executable now that `extn_extfs.cpp` will depend on
`HostVolume`):
...
- `cmake --build --preset macos` — clean.
- `./bld/macos/tests` — all pass.

### Fence

- [ ] `host_volume.cpp` in both main exe and test target
- [ ] Storage sources in main exe `MINIVMAC_SOURCES`
- [ ] Full build clean, all tests pass
- [ ] Commit: `"hostvolume: phase 9 — build integration"`
...
Replace the read-only command handlers in `extn_extfs.cpp` with
`HostVolume` calls.  Write commands stay unchanged until Phase 11.

### 10.1 — Add `s_volume` and error helper
...
Replace `buildCatalog()` with `s_volume.mount("shared")`.  Remove
`buildCatalog()` function.

### 10.3 — Rewrite `kExtFSGetVol`

Delegate to `s_volume.volumeStats()`.

### 10.4 — Rewrite `kExtFSGetCatInfo`

Replace entire case body.  `index > 0` → `s_volume.nthChild()`.
`index == 0` → `s_volume.findByCNID()`.  Virtual root parent (dirID 1)
special case: return root directory (CNID 2, name "Shared").

### 10.5 — Rewrite `kExtFSGetCatInfoName`

Delegate to `s_volume.findByName()`.  Virtual root parent: compare
name to "Shared".

### 10.6 — Rewrite `kExtFSGetFileInfo`

Delegate to `s_volume.findByCNID()` → return type, creator, dates.
Keep virtual root special case.
...
`kExtFSOpen` → `s_volume.openFork()`.
`kExtFSRead` → read guest buffer count, allocate `std::vector<uint8_t>`,
call `s_volume.readFork()`, copy to guest RAM via `put_vm_byte()`.
`kExtFSClose` → `s_volume.closeFork()`.

### Fence

- [ ] All read-only commands delegate to `s_volume`
- [ ] Open/Read/Close delegate to fork I/O
- [ ] `buildCatalog()` and old scan functions removed
- [ ] Debug logging preserved
- [ ] Manual test: emulator boots, Shared volume appears, file listing
      and open work
- [ ] Full build clean, all automated tests pass
- [ ] Commit: `"hostvolume: phase 10 — rewire read-only commands"`
...
Replace write-path command handlers.

### 11.1 — `kExtFSCreateFile`

Read macName from guest, call `s_volume.createFile()`, return CNID.

### 11.2 — `kExtFSWrite`

Read guest buffer into `std::vector<uint8_t>`, call
`s_volume.writeFork()`, return bytes written.

### 11.3 — `kExtFSDeleteFile`

Read macName, call `s_volume.remove()`.

### 11.4 — `kExtFSSetFileInfo`

Call `s_volume.setFileInfo()`.

### 11.5 — `kExtFSCreateDir`

Read macName, call `s_volume.createDir()`, return CNID.

### 11.6 — `kExtFSCatMove`

Read macName + dstDir, call `s_volume.move()`.

### Fence

- [ ] All 6 write commands delegate to `s_volume`
- [ ] Manual test: create/copy/delete files from guest
- [ ] Metadata persists across emulator restart (sidecar-backed)
- [ ] Full build clean, all automated tests pass
- [ ] Commit: `"hostvolume: phase 11 — rewire write commands"`
...
### 12.1 — Delete old types and statics

Remove from `extn_extfs.cpp`:

- Old `struct CatalogEntry`
- `s_catalog`, `s_mounted`, `s_sharedDir`
- `struct OpenFile`, `s_openFiles`, `s_nextHandle`
- `struct WDEntry`, `s_wdTable`, `s_nextWD`
- `s_nextCNID`

### 12.2 — Delete old helper functions

Remove:

- `fourCC()`
- `struct TypeMap`, `s_typeMap[]`, `mapTypeCreator()`
- `toMacRoman()`, `truncateMacName()`
- `toMacDate()`
- `getResourceForkSize()`
- `kMacEpochOffset` (file-local copy)
- `scanDirectory()`, `buildCatalog()`
- `findByCNID()`, `findByNameInDir()`
- `getNthChild()`, `countChildren()`

### 12.3 — Verify

What remains in `extn_extfs.cpp` (~250–300 lines):

- `#include "storage/host_volume.h"` and core headers
- `static storage::HostVolume s_volume`
- `fmErrToReg()` helper
- `readPascalString()`, `writePascalString()`
- `ExtnExtFSDispatch()` switch/case (~200 lines)
- `kExtFSDbgLog`, `kExtFSGuestVars`, `kExtFSFatal` handlers
