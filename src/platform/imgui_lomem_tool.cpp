...
void LowMemTool::draw()
{
	...
	if (ImGui::BeginTable("lomem", 5, flags))
	{
		...
		ImGui::TableSetupColumn(
			"Type", ImGuiTableColumnFlags_NoSort | ImGuiTableColumnFlags_WidthFixed, 50.0f, 2);
		...
		for (int i = 0; i < kLowMemCount && count < 256; i++)
		{
			...
			if (categoryFilter_ > 0 && (int)e.category != categoryFilter_ - 1) continue;
			if (filterBuf_[0] && !contains_ci(e.name, filterBuf_) &&
				!contains_ci(e.brief, filterBuf_))
				continue;
			sortedIdx[count++] = i;
		}
		...
		for (int n = 0; n < count; n++)
		{
			...
			if (ImGui::IsItemHovered()) ImGui::SetTooltip("%s", e.brief);
		}
