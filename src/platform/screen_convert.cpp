...
void BuildPalette()
{
	...
	int depth = (ds.useColorMode && ds.screenDepth > 0 && ds.screenDepth < 4) ? ds.screenDepth : 0;
...
template <int Depth>
void ConvertScreenIndexed(const uint8_t *src, uint32_t *dst, const uint32_t *palette, int width,
						  int height)
{
...
static void ConvertScreenDepth4(const uint8_t *src, uint32_t *dst, int width, int height)
{
...
static void ConvertScreenDepth5(const uint8_t *src, uint32_t *dst, int width, int height)
{
	int nPixels = width * height;
	for (int i = 0; i < nPixels; ++i)
	{
				   ...
				   int height)
{
	...
	{
		case 0:
			ConvertScreenIndexed<0>(src, dst, palette, width, height);
			break;
		case 1:
			ConvertScreenIndexed<1>(src, dst, palette, width, height);
			break;
		case 2:
			ConvertScreenIndexed<2>(src, dst, palette, width, height);
			break;
		case 3:
			ConvertScreenIndexed<3>(src, dst, palette, width, height);
			break;
		case 4:
			ConvertScreenDepth4(src, dst, width, height);
			break;
		case 5:
			ConvertScreenDepth5(src, dst, width, height);
			break;
		default:
			break;
	}
