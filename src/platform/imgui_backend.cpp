...
{
	if (effect_) { effect_->shutdown(); effect_.reset(); }
	if (emuTextureId_)
	{
...
/* ── CRT effect + ImGui drawing ─────────────────────── */

void ImGuiBackend::setEffect(std::unique_ptr<RenderEffect> e)
{
	if (effect_) effect_->shutdown();
	effect_ = std::move(e);
...
void ImGuiBackend::drawViewportFullscreen()
{
	...
	ImVec4 bgColor = effect_ ? effect_->surroundColor()
