...
VIABase::VIABase(int viaNum, uint16_t abnormalBase, int ictTimer1, int ictTimer2)
	: viaNum_(viaNum)
	, abnormalBase_(abnormalBase)
	, ictTimer1_(ictTimer1)
	, ictTimer2_(ictTimer2)
{
}
...
uint8_t VIABase::shiftOutData()
{
	const auto& cfg = viaConfig();
	if (((d_.ACR & 0x1C) >> 2) != 7) {
		ReportAbnormalID(abnormalBase_ | 0x04,
			"VIA Not ready to shift out");
		return 0;
	} else {
		setInterruptFlag(kIntSR);
		setInterruptFlag(kIntCB1);
	...
	switch (addr) {
		...
		case kSR:
			...
			switch ((d_.ACR & 0x1C) >> 2) {
				...
				case 6 : /* shift out under o2 clock */
					if ((! WriteMem) || (d_.SR != 0)) {
						ReportAbnormalID(abnormalBase_ | 0x05,
		...
		case kACR:
			if (WriteMem) {
				...
				d_.ACR = Data;
				if ((d_.ACR & 0x20) != 0) {
					ReportAbnormalID(abnormalBase_ | 0x06,
						"Set d_.ACR T2 Timer pulse counting");
				}
				switch ((d_.ACR & 0xC0) >> 6) {
					case 2:
						ReportAbnormalID(abnormalBase_ | 0x07,
				...
				switch ((d_.ACR & 0x1C) >> 2) {
					...
					case 4:
					case 5:
						ReportAbnormalID(abnormalBase_ | 0x08,
				...
				if ((d_.ACR & 0x03) != 0) {
					ReportAbnormalID(abnormalBase_ | 0x09,
...
#define SetContains(s, i) (((s) & (1 << (i))) != 0)
				if (! SetContains(acfg.cb2ModesAllowed,
					(d_.PCR >> 5) & 0x07))
				{
					ReportAbnormalID(abnormalBase_ | 0x0A,
						"Set d_.PCR CB2 Control mode?");
				}
				if ((d_.PCR & 0x10) != 0) {
					ReportAbnormalID(abnormalBase_ | 0x0B,
						"Set d_.PCR CB1 INTERRUPT CONTROL?");
				}
				if (! SetContains(acfg.ca2ModesAllowed,
					(d_.PCR >> 1) & 0x07))
				{
					ReportAbnormalID(abnormalBase_ | 0x0C,
						"Set d_.PCR CA2 INTERRUPT CONTROL?");
				}
				if ((d_.PCR & 0x01) != 0) {
					ReportAbnormalID(abnormalBase_ | 0x0D,
				...
				if ((Data & 0x80) == 0) {
					...
					if (acfg.ierNever0 != 0) {
						if ((Data & acfg.ierNever0) != 0) {
							ReportAbnormalID(abnormalBase_ | 0x0E,
								"IER Never0 clr");
						}
					}
				} else {
					...
					if (acfg.ierNever1 != 0) {
						if ((d_.IER & acfg.ierNever1) != 0) {
							ReportAbnormalID(abnormalBase_ | 0x0F,
								"IER Never1 set");
						}
					}
				}
