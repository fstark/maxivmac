...
static void ApplyScreenHack(uint8_t *pto)
{
	...
	if (g_machine->config().model <= MacModel::Mac128K)
	{
		...
		do_put_mem_long(292 + g_rom,
						kVidMem_Base + (((vMacScreenHeight / 4) * 2 + 9) * vMacScreenWidth +
		...
		do_put_mem_word(456 + g_rom, (vMacScreenHeight * vMacScreenWidth / 32) - 1 + 32);
		...
		do_put_mem_long(1388 + g_rom,
						kVidMem_Base + (((vMacScreenHeight / 4) * 2 - 25) * vMacScreenWidth +
										(vMacScreenWidth / 2 - 16)) /
										   8);
		/* blink floppy, question mark */
		do_put_mem_long(1406 + g_rom,
						kVidMem_Base + (((vMacScreenHeight / 4) * 2 - 10) * vMacScreenWidth +
										(vMacScreenWidth / 2 - 8)) /
										   8);
		...
		do_put_mem_long(2020 + g_rom,
						kVidMem_Base + (((vMacScreenHeight / 4) * 2 - 19) * vMacScreenWidth +
										(vMacScreenWidth / 2 - 8)) /
										   8);
	...
	else if (g_machine->config().model <= MacModel::Plus)
	{
		...
		do_put_mem_long(420 + g_rom,
						kVidMem_Base + (((vMacScreenHeight / 4) * 2 + 17) * vMacScreenWidth +
										(vMacScreenWidth / 2 - 8)) /
										   8);

		do_put_mem_word(494 + g_rom, (vMacScreenHeight * vMacScreenWidth / 32) - 1);
