...
static void SetDcoArgFields(WorkR *p, bool src, uint8_t CurAMd, uint8_t CurArgDat)
{
	if (src)
	{
...
static bool CheckValidAddrMode(WorkR *p, uint8_t m, uint8_t r, uint8_t v, bool src)
{
	...
	switch (m)
	{
		case 0:
			...
			CurArgDat = r;
			IsOk = CheckInSet(v, kAddrValidMaskAny | kAddrValidMaskData | kAddrValidMaskDataAlt |
		...
		case 2:
			...
			CurArgDat = r + 8;
			IsOk = CheckInSet(v, kAddrValidMaskAny | kAddrValidMaskData | kAddrValidMaskDataAlt |
									 kAddrValidMaskControl | kAddrValidMaskControlAlt |
									 kAddrValidMaskAltMem | kAddrValidMaskDataNoCn);
			break;
		case 3:
			switch (p->opsize)
			{
		...
		case 7:
			switch (r)
			{
				case 0:
					...
					IsOk = CheckInSet(v, kAddrValidMaskAny | kAddrValidMaskData |
				...
				case 2:
					...
					IsOk = CheckInSet(v, kAddrValidMaskAny | kAddrValidMaskData |
											 kAddrValidMaskControl | kAddrValidMaskDataNoCn);
					break;
				case 3:
					switch (p->opsize)
					{
...
{
	return CheckValidAddrMode(p, mode(p), reg(p), kAddrValidAny, false);
}

static bool CheckDataAltAddrMode(WorkR *p)
{
	return CheckValidAddrMode(p, mode(p), reg(p), kAddrValidDataAlt, false);
}

static bool CheckDataAddrMode(WorkR *p)
{
	return CheckValidAddrMode(p, mode(p), reg(p), kAddrValidData, false);
}

static bool CheckControlAddrMode(WorkR *p)
{
	return CheckValidAddrMode(p, mode(p), reg(p), kAddrValidControl, false);
}

static bool CheckControlAltAddrMode(WorkR *p)
{
	return CheckValidAddrMode(p, mode(p), reg(p), kAddrValidControlAlt, false);
}

static bool CheckAltMemAddrMode(WorkR *p)
{
	return CheckValidAddrMode(p, mode(p), reg(p), kAddrValidAltMem, false);
}
			...
			else
			{
				switch (rg9(p))
				{
					case 0:
						FindOpSizeFromb76(p);
						if (CheckValidAddrMode(p, 7, 4, kAddrValidAny, true))
							if (CheckValidAddrMode(p, mode(p), reg(p), kAddrValidDataAlt, false))
							{
								if (0 != mode(p))
								{
									p->Cycles = (4 == p->opsize)
			...
			if (mode(p) < 2)
			{
				switch (b76(p))
				{
					...
					case 1:
						...
						{
							if (CheckValidAddrMode(p, 1, rg9(p), kAddrValidAny, true))
