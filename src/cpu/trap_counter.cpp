...
static constexpr int kDictSize =
	static_cast<int>(sizeof(s_dict) / sizeof(s_dict[0]));
...
int trap_dict_size() { return kDictSize; }
	...
	int maxResults)
{
	...
	for (int i = 0; i < kDictSize && (int)results.size() < maxResults; ++i) {
		if (ciStartsWith(s_dict[i].name, prefix))
			results.push_back(s_dict[i]);
	}
}
...
void trap_watch_remove(uint16_t trapWord)
{
	...
	if (it != s_watchlist.end())
		s_watchlist.erase(it);
}
