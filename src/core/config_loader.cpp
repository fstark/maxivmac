...
MachineConfig BuildMachineConfig(const LaunchConfig &launch)
{
	...
	*/
	if (!config.includeVidMem && !config.isIIFamily() && config.model != MacModel::PB100)
	{
		bool needScreenHack = config.screenWidth != 512 || config.screenHeight != 342;
		if (needScreenHack)
		{
			uint32_t fbSize =
				(((uint32_t)config.screenWidth * config.screenHeight << config.screenDepth) + 7) >>
				3;
	...
	*/
	if (config.includeVidMem && config.isIIFamily())
	{
		...
		fbSize |= fbSize >> 16;
		fbSize++;
		if (fbSize > config.vidMemSize) config.vidMemSize = fbSize;
	}
...
{
	if (!romPath.empty()) return romPath;
