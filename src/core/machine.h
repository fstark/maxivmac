...
extern uint8_t *get_real_address0(uint32_t L, bool WritableMem, uint32_t addr, uint32_t *actL);
...
extern void dbglog_AddrAccess(char *s, uint32_t data, bool writeMem, uint32_t addr);
...
constexpr int kATTA_readwritereadymask = (1 << kATTA_readreadybit) | (1 << kATTA_writereadybit);
