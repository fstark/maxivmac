...
void CustomReset()
{
	...
	if (auto *d = g_rig->findDevice<VIA1Device>()) d->reset();
	if (auto *d = g_rig->findDevice<VIA2Device>()) d->reset();
	if (auto *d = g_rig->findDevice<SonyDevice>()) d->reset();
	ExtnReset();
	if (g_rig->config().isCompactMac())
	{
		g_wantMacReset = true;
		/*
...
#if WantAbnormalReports
void DoReportAbnormalID(uint16_t id, char *s)
{
	dbglog_StartLine();
...
static uint32_t GetRAMLn2Spc()
{
	...
	if (m == MacModel::PB100 || g_rig->config().isIIFamily()) return 23;
	return 22;
}

static uint32_t GetVidMemBase()
{
	if (g_rig->config().model == MacModel::PB100) return 0x00FA0000;
	return 0x00540000;
}
static uint32_t GetVidMemLn2Spc()
{
	if (g_rig->config().model == MacModel::PB100) return 16;
	return 18;
}

static uint32_t GetSCSIBlockBase()
{
	if (g_rig->config().model == MacModel::PB100) return 0x00F90000;
	return 0x00580000;
}
...
{
	if (g_rig->config().model == MacModel::PB100) return 0x00FD0000;
	return 0x00800000;
}
...
{
	if (g_rig->config().model == MacModel::PB100) return 0x00F60000;
	return 0x00C00000;
}
...
{
	if (g_rig->config().model == MacModel::PB100) return 0x00F70000;
	return 0x00E80000;
}
...
{
	if (g_rig->config().model == MacModel::PB100) return 0x00FB0000;
...
static tMacErr PbufTransferVM(uint32_t buffera, PbufIndex i, uint32_t offset, uint32_t count,
							  bool isWrite)
{
	uint32_t contig;
	uint8_t *buffer;
	...
	switch (get_vm_word(p + EXTN_DAT_COMMND))
	{
		...
		case kCmndPbufTransfer:
		{
			...
			if (tMacErr::noErr == result)
			{
				...
				{
					result = PbufTransferVM(buffera, pbufNo, offset, count, isWrite);
				}
			}
		}
		break;
...
static void ExtnFind_Access(uint32_t p)
{
	...
	switch (get_vm_word(p + EXTN_DAT_COMMND))
	{
		...
		case kCmndFindExtnFind:
		{
			...
			else if (extn == kHostParamBuffersExtension)
			{
				put_vm_word(p + kParamFindExtnTheId, kExtnParamBuffers);
				result = tMacErr::noErr;
			}
			else if (extn == kHostClipExchangeExtension)
			{
		...
		case kCmndFindExtnId2Code:
		{
			...
			else if (extn == kExtnParamBuffers)
			{
				put_vm_long(p + kParamFindExtnTheExtn, kHostParamBuffersExtension);
				result = tMacErr::noErr;
			}
			else if (extn == kExtnHostTextClipExchange)
			{
...
static uint32_t regBlockAccess(uint32_t data, bool writeMem, uint32_t regOff)
{
	...
	if (regOff >= 2 && regOff < 26)
	{
		...
		if (writeMem)
		{
			if (isLow)
			{
				s_regParam[paramIdx] = (s_regParam[paramIdx] & 0xFFFF0000u) | (data & 0xFFFF);
			}
			else
			{
				s_regParam[paramIdx] =
					(s_regParam[paramIdx] & 0x0000FFFFu) | ((data & 0xFFFF) << 16);
			}
		}
		if (isLow)
		{
...
static uint32_t extnAccess(uint32_t Data, bool writeMem, uint32_t addr)
{
	...
	switch (addr)
	{
			...
			if (kcom_callcheck == get_vm_word(p + EXTN_DAT_CHECKVAL))
			{
				...
				switch (get_vm_word(p + EXTN_DAT_EXTENSION))
				{
					case kExtnFindExtn:
						...
						break;
					case kExtnVideo:
						if (auto *d = g_rig->findDevice<VideoDevice>()) d->extnVideoAccess(p);
						break;
					...
					case kExtnHostTextClipExchange:
						...
						break;
					case kExtnDisk:
						if (auto *d = g_rig->findDevice<SonyDevice>()) d->extnDiskAccess(p);
						break;
					case kExtnSony:
						if (auto *d = g_rig->findDevice<SonyDevice>()) d->extnSonyAccess(p);
						break;
					default:
		...
		case kDSK_QuitOnEject:
			...
			if (auto *d = g_rig->findDevice<SonyDevice>()) d->setQuitOnEject();
			break;
	}
	return Data;
}
	...
	if (cfg.ramASize == cfg.ramBSize)
	{
		...
		else
		{
			...
			if (0 != bankbit)
			{
				if (cfg.ramBSize != 0)
				{
					r.cmpmask = bankbit | (0x00FFFFFF & ~((1 << GetRAMLn2Spc()) - 1));
					r.cmpvalu = bankbit;
...
static void SetUp_io()
{
	...
	r.MMDV = kMMDV_VIA1;
	r.device = g_rig->findDevice<VIA1Device>();
	AddToATTList(&r);
	...
	r.Access = kATTA_mmdvmask;
	r.MMDV = kMMDV_VIA2;
	r.device = g_rig->findDevice<VIA2Device>();
	AddToATTList(&r);
	...
	r.Access = kATTA_mmdvmask;
	r.MMDV = kMMDV_SCC;
	r.device = g_rig->findDevice<SCCDevice>();
	AddToATTList(&r);
	...
	r.Access = kATTA_mmdvmask;
	r.MMDV = kMMDV_SCSI;
	r.device = g_rig->findDevice<SCSIDevice>();
	AddToATTList(&r);
	...
	r.Access = kATTA_mmdvmask;
	r.MMDV = kMMDV_ASC;
	r.device = g_rig->findDevice<ASCDevice>();
	AddToATTList(&r);
	...
	r.Access = kATTA_mmdvmask;
	r.MMDV = kMMDV_IWM;
	r.device = g_rig->findDevice<IWMDevice>();
	AddToATTList(&r);
}
...
static void SetUp_address_compact()
{
	...
	if (g_rig->config().includeVidMem)
	{
		...
		r.usemask = g_rig->config().vidMemSize - 1;
		r.usebase = g_vidMem;
		r.Access = kATTA_readwritereadymask;
		AddToATTList(&r);
	}
	...
	r.cmpmask = 0x00FFFFFF & ~((1 << g_rig->config().extnLn2Spc) - 1);
	r.cmpvalu = g_rig->config().extnBlockBase;
	r.usebase = nullptr;
	r.Access = kATTA_mmdvmask;
	r.MMDV = kMMDV_Extn;
	r.device = &g_extnDevice;
	...
	if (g_rig->config().model == MacModel::PB100)
	{
...
uint32_t MMDV_Access(ATTEntryPtr p, uint32_t data, bool writeMem, bool byteSize, uint32_t addr)
{
	...
	switch (p->MMDV)
	{
		case kMMDV_VIA1:
			if (!byteSize)
			{
				if (g_rig->config().isIIFamily() && writeMem && (addr == 0xF40006))
				{
		...
		case kMMDV_VIA2:
			if (!byteSize)
			{
				if ((!writeMem) && ((0x3e00 == (addr & 0x1FFFF)) || (0x3e02 == (addr & 0x1FFFF))))
				{
					...
					data = (p->device->access(data, writeMem, (addr >> 9) & kVIA2_Mask) << 8) |
						   p->device->access(data, writeMem, (addr >> 9) & kVIA2_Mask);
				}
				else
				{
		...
		case kMMDV_SCC:
			...
			else if (!g_rig->config().isIIFamily() && writeMem != ((addr & 1) != 0))
			{
				if (writeMem)
				{
					auto m = g_rig->config().model;
			...
			else if (g_rig->config().model != MacModel::PB100 && !g_rig->config().isIIFamily() &&
		...
		case kMMDV_ASC:
			if (!byteSize)
			{
				if (g_rig->config().isIIFamily())
				{
					if (writeMem)
					{
						(void)p->device->access((data >> 8) & 0x00FF, writeMem, addr & kASC_Mask);
						data = p->device->access((data) & 0x00FF, writeMem, (addr + 1) & kASC_Mask);
					}
					else
					{
						data = (p->device->access((data >> 8) & 0x00FF, writeMem, addr & kASC_Mask)
								<< 8) |
							   p->device->access((data) & 0x00FF, writeMem, (addr + 1) & kASC_Mask);
					}
				}
				else
				{
		...
		case kMMDV_IWM:
			...
			else
			{
				...
				{
					if (g_rig->config().model != MacModel::PB100 && !g_rig->config().isIIFamily())
					{
...
bool MemAccessNtfy(ATTEntryPtr pT)
{
	...
	switch (pT->Ntfy)
	{
		case kMAN_OverlayOff:
			if (g_rig->config().model >= MacModel::SE)
			{
...
uint8_t *get_real_address0(uint32_t L, bool WritableMem, uint32_t addr, uint32_t *actL)
{
	uint32_t bankleft;
	uint8_t *p;
	ATTEntryPtr q;
...
void VIAorSCCinterruptChngNtfy()
{
	...
	else
	{
		...
		NewIPL = VIAandNotSCC | (SCCInterruptRequest << 1) | (g_interruptButton << 2);
	}
	if (NewIPL != s_curIPL)
	{
		s_curIPL = NewIPL;
...
bool AddrSpac_Init()
{
	...
	if (g_rig->config().emRTC)
	{
		g_wires.onChange(Wire_VIA1_iB0_RTCdataLine,
						 []()
						 {
							 if (auto *d = g_rig->findDevice<RTCDevice>()) d->dataLineChangeNtfy();
						 });
		g_wires.onChange(Wire_VIA1_iB1_RTCclock,
						 []()
						 {
							 if (auto *d = g_rig->findDevice<RTCDevice>()) d->clockChangeNtfy();
						 });
		g_wires.onChange(Wire_VIA1_iB2_RTCunEnabled,
						 []()
						 {
							 if (auto *d = g_rig->findDevice<RTCDevice>()) d->unEnabledChangeNtfy();
						 });
	}
	if (g_rig->config().emADB)
	{
		g_wires.onChange(Wire_VIA1_iB4_ADB_st0,
						 []()
						 {
							 if (auto *d = g_rig->findDevice<ADBDevice>()) d->stateChangeNtfy();
						 });
		g_wires.onChange(Wire_VIA1_iB5_ADB_st1,
						 []()
						 {
							 if (auto *d = g_rig->findDevice<ADBDevice>()) d->stateChangeNtfy();
						 });
		g_wires.onChange(Wire_VIA1_iCB2_ADB_Data,
						 []()
						 {
							 if (auto *d = g_rig->findDevice<ADBDevice>()) d->dataLineChngNtfy();
						 });
	}
	if (g_rig->config().emClassicKbrd)
	{
		g_wires.onChange(Wire_VIA1_iCB2,
						 []()
						 {
	...
	{
		g_wires.onChange(Wire_PMU_ToReady,
						 []()
						 {
							 if (auto *d = g_rig->findDevice<PMUDevice>()) d->toReadyChangeNtfy();
						 });
	}

	g_cpu.init(&s_curIPL, &g_rig->config());
	return true;
}
