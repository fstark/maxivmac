...
public:
	...
	void cpu(uint32_t instructionCount, uint32_t pc, uint16_t sr, const uint32_t d[8],
			 const uint32_t a[8]);

	// ── Per-I/O-access hook (MMDV_Access) ──
	void io(uint32_t instructionCount, uint32_t address, uint32_t data, bool write, bool byteSize,
			const char *deviceName);

	bool active() const;

private:
	void cpuSlow(uint32_t instructionCount, uint32_t pc, uint16_t sr, const uint32_t d[8],
				 const uint32_t a[8]);
	void ioSlow(uint32_t instructionCount, uint32_t address, uint32_t data, bool write,
				bool byteSize, const char *deviceName);
	void textIo(uint32_t icount, uint32_t addr, uint32_t data, bool write, bool byteSize,
				const char *device);
	void finish();
