...
bool StateRecorder::init(const Config& cfg)
{
	...
	if (mode_ == RecorderMode::Off && textLog_ == TextLog::None)
		return true;
	...
	if (mode_ == RecorderMode::Record) {
		...
		if (h.screenWidth != cfg.screenWidth || h.screenHeight != cfg.screenHeight) {
			std::fprintf(stderr, "StateRecorder: screen size mismatch: golden=%ux%u, current=%ux%u\n",
		...
		// disk hash: only check if golden has a non-zero hash
		{
			...
			if (std::memcmp(h.diskHash, zero, 16) != 0 &&
				...
				std::fprintf(stderr, "StateRecorder: disk hash mismatch:\n  golden: %s\n  actual: %s\n",
					expH, gotH);
				std::fclose(f);
				return false;
			}
		...
		if (dataSize < 0 || (dataSize % sizeof(CpuSnapshot)) != 0) {
			std::fprintf(stderr, "StateRecorder: file size inconsistent "
...
static void dumpCpuSnapshot(FILE* out, const CpuSnapshot& snap)
{
	std::fprintf(out,
		...
		" IO_CRC=%08X\n",
		snap.instructionCount, snap.pc, snap.sr,
		snap.d[0], snap.d[1], snap.d[2], snap.d[3],
		snap.d[4], snap.d[5], snap.d[6], snap.d[7],
		snap.a[0], snap.a[1], snap.a[2], snap.a[3],
		snap.a[4], snap.a[5], snap.a[6], snap.a[7],
		snap.ioCrc);
	std::fflush(out);
}

static void dumpMismatch(FILE* out, uint32_t snapIdx,
                         const CpuSnapshot& expected, const CpuSnapshot& actual)
{
	std::fprintf(out,
...
void StateRecorder::cpuSlow(uint32_t instructionCount,
                            ...
                            const uint32_t d[8], const uint32_t a[8])
{
	...
	if (mode_ == RecorderMode::Record) {
		...
		if (textLog_ >= TextLog::CpuOnly)
			dumpCpuSnapshot(out, snap);
		...
		bool cpuOk = (snap.instructionCount == expected.instructionCount &&
		              snap.pc == expected.pc &&
		              snap.sr == expected.sr &&
		              std::memcmp(snap.d, expected.d, sizeof(snap.d)) == 0 &&
		...
		if (!cpuOk) {
			...
			std::fprintf(out, "IO_CRC warning at snapshot #%u (insn %" PRIu64 "): "
				"expected %08X, actual %08X\n",
				snapshotIndex_, snap.instructionCount,
				expected.ioCrc, snap.ioCrc);
		}
...
void StateRecorder::ioSlow(uint32_t instructionCount,
                           uint32_t address, uint32_t data,
                           bool write, bool byteSize, const char* deviceName)
{
	struct { uint32_t ic; uint32_t addr; uint32_t data; uint8_t dir; uint8_t bs; } rec;
...
void StateRecorder::textIo(uint32_t icount, uint32_t addr, uint32_t data,
                           bool write, bool byteSize, const char* device)
{
	FILE* out = textFile_ ? textFile_ : stderr;
	if (byteSize)
		std::fprintf(out, "IO  @%u %c.b %-4s %08X %02X\n",
			icount, write ? 'W' : 'R', device, addr, data & 0xFF);
	else
		std::fprintf(out, "IO  @%u %c.w %-4s %08X %04X\n",
			icount, write ? 'W' : 'R', device, addr, data & 0xFFFF);
}
...
void StateRecorder::cpu(uint32_t instructionCount,
                        uint32_t pc, uint16_t sr,
                        const uint32_t d[8], const uint32_t a[8])
{
...
void StateRecorder::io(uint32_t instructionCount,
                       uint32_t address, uint32_t data,
                       bool write, bool byteSize, const char* deviceName)
{
