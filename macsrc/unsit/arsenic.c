...
typedef struct { int symbol, frequency; } ArithSym;
...
static void arith_model_init(ArithModel *m, int first, int last,
...
#define ARITH_ONE   (1 << (ARITH_BITS - 1))
#define ARITH_HALF  (1 << (ARITH_BITS - 2))
...
static int arith_bitstring(ArithDecoder *dec, ArithModel *m, int bits)
{
    int res = 0, i;
    for (i = 0; i < bits; i++)
        if (arith_symbol(dec, m))
            res |= 1 << i;
    return res;
}
...
int decompress_arsenic(UFile *uf, u8 *outbuf, s32 length)
{
    ...
    /* Stream header */
    if (arith_bitstring(&dec, &initial, 8) != 'A' ||
        arith_bitstring(&dec, &initial, 8) != 's') {
        fprintf(stderr, "unsit: arsenic: bad stream header\n");
        return -1;
    }

    blockbits = arith_bitstring(&dec, &initial, 4) + 9;
                ...
                if (bytecount >= numbytes) {
                    ...
                    for (;;) {
                        ...
                        if (sel == 0 || sel == 1) {
                            ...
                            memset(&block[numbytes],
                                   (u8)mtf_decode(&mtf, 0), (size_t)zerocount);
                            numbytes += zerocount;
                        }
...

                        {
                            int symbol;
                            if (sel == 2) symbol = 1;
                            else symbol = arith_symbol(&dec, &mtfm[sel - 3]);
                            if (numbytes >= blocksize) { rc = -1; goto done; }
                            block[numbytes++] = (u8)mtf_decode(&mtf, symbol);
                        }
                    }

                    if (transformindex >= numbytes) { rc = -1; goto done; }

                    arith_model_reset(&selector);
                    for (i = 0; i < 7; i++) arith_model_reset(&mtfm[i]);
    ...
    if (compcrc != 0 && compcrc != crc) {
        fprintf(stderr, "unsit: arsenic: CRC mismatch "
                "(expected %08x, got %08x)\n",
                (unsigned)compcrc, (unsigned)crc);
    }
