...
static char path_stack[MAX_DEPTH][36]; /* Mac filenames ≤ 31 chars */
...
static void path_push(const char *name)
{
	...
	{
		snprintf(path_stack[path_depth], sizeof(path_stack[0]), "%s", name);
		path_depth++;
	}
}
...
static void path_build(char *buf, int bufsz)
{
	...
	{
		int n = snprintf(buf + pos, bufsz - pos, "%s%s", (i > 0) ? "/" : "", path_stack[i]);
		pos += n;
		if (pos >= bufsz) break;
	}
}
...
int sit1_parse(UFile *uf, EntryCallback cb, void *ctx)
{
	...
	uf_seek(uf, 0);
	if (uf_read(uf, archdr, 22) != 0) return -1;
	...
	while (uf_tell(uf) + SIT_FILEHDRSIZE <= totalsize)
	{
		...
		if (uf_read(uf, hdr, SIT_FILEHDRSIZE) != 0) break;
		...
		{
			fprintf(stderr,
					"unsit: classic: header CRC mismatch "
			...
			(rsrc_method & StuffItFolderMask) == StuffItStartFolder)
		{
			...
			if (cb(path_buf, &entry, uf, ctx) != 0) return -1;
