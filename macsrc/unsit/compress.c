...
static void lzw_init(LZWState *st, int flags)
{
    ...
    if (st->maxsyms > LZW_MAX_SYMBOLS)
        st->maxsyms = LZW_MAX_SYMBOLS;
...
static int lzw_output(LZWState *st, int sym, u8 *buf, int bufsz)
{
    int n = 0, s = sym;
    /* Count length */
    while (s >= 0) { n++; s = st->nodes[s].parent; }
    if (n > bufsz) n = bufsz;
    /* Fill backwards */
    {
        int pos = n;
...
typedef struct { UFile *uf; u32 buf; int bits; } BrLE2;
