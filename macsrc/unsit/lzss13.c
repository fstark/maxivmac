...
typedef struct { UFile *uf; u32 buf; int bits; } BrLE;

static void brle_init(BrLE *br, UFile *uf)
{ br->uf = uf; br->buf = 0; br->bits = 0; }
...
typedef struct { int branches[2]; } TNode;
typedef struct { s16 value; s16 length; } TblEntry;
...
static void pc_insert(PCode *pc, int sym, u32 code, int len)
{
    ...
    for (i = len - 1; i >= 0; i--) {
        int bit = (code >> i) & 1;
        if (pc->nodes[node].branches[bit] < 0)
            pc->nodes[node].branches[bit] = pc_alloc(pc);
...
static void pc_fill_tbl(PCode *pc, int node, TblEntry *t, int depth, int maxd)
{
    ...
    if (node < 0) {
        for (i = 0; i < sz; i++) { t[i*stride].value = -1; t[i*stride].length = -1; }
    } else if (IS_LEAF(pc->nodes[node])) {
        int sym = LEAF_SYM(pc->nodes[node]);
        for (i = 0; i < sz; i++) { t[i*stride].value = (s16)sym; t[i*stride].length = (s16)depth; }
    } else if (depth == maxd) {
        t[0].value = (s16)node;
...
static void pc_build_explicit(PCode *pc, const u32 *codes,
...
static const int MetaCodeLens[37] = {
...
int decompress_lzss13(UFile *uf, u8 *outbuf, s32 length)
{
    ...
    if (!fc || !sc || !oc || !window) { rc = -1; goto done; }
    ...
    if (code == 0) {
        ...
        if (val & 0x08) { free(sc); sc = fc; }
        else parse_dynamic(sc, &br, &meta, 321);
    ...
    while (outpos < length) {
        ...
        if (val < 0x100) {
            ...
            if (val < 0x13e)       mlen = val - 0x100 + 3;
            else if (val == 0x13e) mlen = (int)brle_bits(&br, 10) + 65;
            else if (val == 0x13f) mlen = (int)brle_bits(&br, 15) + 65;
            else break; /* 0x140 = end */

            bl = pc_decode(oc, &br);
            if (bl < 0) { rc = -1; break; }
            if (bl == 0)      off = 1;
            else if (bl == 1) off = 2;
            else              off = (1 << (bl-1)) + (int)brle_bits(&br, bl-1) + 1;
