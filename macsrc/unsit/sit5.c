...
typedef struct {
    ...
    char path[1024];        /* accumulated path prefix */
} DirEntry;

static DirEntry dir_table[MAX_DIRS];
...
static void dir_add(s32 offset, const char *path)
{
    if (dir_count < MAX_DIRS) {
        ...
        snprintf(dir_table[dir_count].path,
                 sizeof(dir_table[0].path), "%s", path);
        dir_count++;
    }
}
...
static const char *dir_lookup(s32 offset)
{
    ...
    for (i = 0; i < dir_count; i++) {
        if (dir_table[i].offset == offset)
            return dir_table[i].path;
    }
    return "";
}
