	...
	unsigned long eof, unsigned char flags)
{
	...
	for (i = 2; i < fcbLen; i += kFCBLen) {
		...
		if (*(long *)(fcb + kFCBFlNum) == 0) {
			/* Zero the entire FCB to clear stale data from
			   previous occupants (extent records, clump size,
			   catalog hints, etc.) */
			{
				short z;
				for (z = 0; z < kFCBLen; z++)
...
static Boolean IsOurVolume(short vRefNum)
{
	...
	if (vRefNum < kOurVRefNum && vRefNum > -33000) return true;
	/* Note: we intentionally do NOT claim vRefNum==0 (default volume).
	   If we did, _SetVol to our volume would make us the default,
	   and all subsequent vRefNum=0 lookups (e.g. finding the Finder)
	   would be routed to us and fail. */
	return false;
}
...
short DispatchFlat(char *pb, short trapWord)
{
		...
		case 0x12: /* _SetEOF */
			...
			if (!IsOurFCB(refNum)) { RestoreA4(); return 1; }
			{
				...
				dbg_log2(g->regBase,
					...
					(long)refNum, newEOF);
				if (fcb != NULL) {
					long handle = *(long *)(fcb + kFCBHostHandle);
					*(long *)(fcb + kFCBEOF) = newEOF;
					*(long *)(fcb + kFCBPLen) = newEOF;
					/* Tell host to resize the fork */
					reg_set(g->regBase, 0,
						(unsigned long)handle);
					reg_set(g->regBase, 1,
						(unsigned long)newEOF);
					reg_command(g->regBase, 0x0218);
				}
			}
	...
	if (!IsOurVolume(vRefNum)) {
		...
		if (trapNum == 0x15 && nameAddr != 0) {
			...
			{
				dbg_log(g->regBase,
					"SD _SetVol by name -> noErr (no DefVCBPtr)");
				*(short *)(pb + pb_ioResult) = 0;
				RestoreA4(); return 0;
			}
		}
	...
	switch (trapNum) {
		...
		case 0x15: /* _SetVol */
			/* Accept _SetVol but do NOT modify DefVCBPtr.
			   If we became the default volume, all subsequent
			   vRefNum=0 calls (e.g. finding the Finder) would
			   be routed to us and fail. */
			dbg_log1(g->regBase,
				"SD _SetVol vr=%ld -> noErr (no DefVCBPtr)",
				(long)vRefNum);
			*(short *)(pb + pb_ioResult) = 0;
			RestoreA4(); return 0;
